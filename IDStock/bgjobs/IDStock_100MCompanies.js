...
async.each(requestURLs,
    ...
    function (err) {
        ...
        async.series({
            ...
            /*
             * Step4: update the DB ticker_price
             */
            step4: function(cblevel2){
                outPutFilteredDataToDB_ticker_price(cblevel2);
            }
...
function cleansingCompanyData(cblevel2){
	for (var i = 0, len = companyArray.length; i < len; i++) {
        ...
        if(makcap.charAt(0) === '$'){
        	...
        	else if ((lastChar === 'M')&&(parseFloat(makcapval) >= 100)){
        		var filteredCompanyData = {symbol:companyArray[i].symbol.trim(), name:companyArray[i].name,
...
function outPutFilteredDataToDB_ticker_price(cblevel2) {
    var queryString = "insert into ticker_price (symbol, price, last_update) values ";
    for (var i = 0, len = companyArrayCleansed.length; i < len; i++) {
    	queryString = queryString + "('" + companyArrayCleansed[i].symbol + "', 0 ,'0000-00-00'),";
    }
    queryString = queryString.substr(0, queryString.length - 1);
