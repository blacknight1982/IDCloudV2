...
async.series({
        ...
        zero1: function (cblevel1) {
            mySQLPool.getConnection(function (err, conn) {
                if (err) throw err;
                var queryString = "delete from analysis_breakout";
        ...
        zero2: function (cblevel1) {
            mySQLPool.getConnection(function (err, conn) {
                if (err) throw err;
                var queryString = "delete from analysis_magather";
                conn.query(queryString, function (error, results) {
                    if (error) {
                        console.log(error);
                    }
        ...
        one: function (cblevel1) {
            ...
            fs.createReadStream('US_Tickers_Selected.csv').pipe(csvStream)
                            ...
                            async.series({
                                    one: function (cblevel3) {
                                        mySQLPool.getConnection(function (err, conn) {
                                            ...
                                            var queryString = "insert into analysis_breakout (symbol, highprice, highprice_date) values ('" +
                                    ...
                                    two: function (cblevel3) {
                                        if (magatherarray.length > 0) {
                                            mySQLPool.getConnection(function (err, conn) {
                                                ...
                                                var queryString = "insert into analysis_magather (symbol, magather_date, magather_price, ma5, ma10, ma20, ma60, ma200) values ";
...
function calculateMovingAveragesAndBreakout(tickerDataArray, magatherarray, breakoutindicator) {
    for (var i = 0, len = tickerDataArray.length; i < len; i++) {
        ...
        if (i < analysisParam.maGatherAttentionPeriod) {
            var maxMA = Math.max(tickerDataArray[i].ma5, tickerDataArray[i].ma10, tickerDataArray[i].ma20, tickerDataArray[i].ma60);
            var minMA = Math.min(tickerDataArray[i].ma5, tickerDataArray[i].ma10, tickerDataArray[i].ma20, tickerDataArray[i].ma60);
            if (((maxMA / minMA) - 1) * 100 < analysisParam.maGatherPercent && tickerDataArray[i].close > tickerDataArray[i].ma200) {
