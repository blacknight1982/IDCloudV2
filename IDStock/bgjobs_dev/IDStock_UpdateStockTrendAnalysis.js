/**
 * Created by John Liu on 05/13/2017.
 * Not finished yet
 */
...
var IDStock_UpdateStockTrendAnalysis = function(){
	
	async.series({
		/*
		 * Step 1 read all company symbols from DB company_basic (market cap > 100M)
		 */
	    step1: function(cbGlobal){
	        
	    	var queryString = 'SELECT symbol FROM company_basic where market_cap > 100';
	    	...
	    	db.get().query(queryString, function (error, rows, results) {
	            if (error) {
	            	...
	            	symbolArray = rows;
	            	symbolArray = symbolArray.slice(0,20);
	    ...
	    /*
		 * Step 2 select symbols with trend has not be analyzed  
		 */
	    ...
	    /*
	     * Step3: Company Stock Trend Analysis for each company in symbolArray
	     */
	    step3: function(cbGlobal){
	        queryPriceHistoryAndTrendAnalysis(cbGlobal);
	    }
...
function queryPriceHistoryAndTrendAnalysis(cbGlobal){
...
module.exports = IDStock_UpdateStockTrendAnalysis;